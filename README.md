...
![HUD 화면](docs/images/HUD.png)
- 일반 설정에서 노트 레이더 HUD 표시 옵션을 활성화하여 적용할 수 있습니다.
- 갱신 과정에서 DLL이 이식되며, 위 사진과 같이 선곡 창에서 해당 곡/난이도의 노트 레이더 그래프가 게임 상에 표시됩니다.
- 본인이 플레이 혹은 다운로드한 기록이 없을 경우 노트 레이더가 제대로 표시되지 않을 수 있으며, 대부분의 경우 다운로드 및 1회 플레이 후 정상적으로 표시됩니다.
- 실험적인 기능이며, DLL 이식이라는 방식 특성 상 계정 제재의 요소가 될 수 있으므로 주의 바랍니다.
